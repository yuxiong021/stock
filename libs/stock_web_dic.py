...
STOCK_WEB_DATA_LIST.append(
    StockWebData(
        ...
        # columns=["ts_code", "name", "industry", "area", "pe", "float_share", "total_share", "totalAssets", "liquidAssets",
        #          "fixedAssets", "reserved", "reservedPerShare", "esp", "bvps", "pb", "timeToMarket",
        #          "undp", "perundp", "rev", "profit", "gpr", "npr", "holders"],
        # column_names=["代码", "名称", "所属行业", "地区", "市盈率", "流通股本(万股)", "总股本(万股)", "总资产(万)", "流动资产",
        #               "固定资产", "公积金", "每股公积金", "每股收益", "每股净资", "市净率", "上市日期", "未分利润",
        #               "每股未分配", "收入同比(%)", "利润同比(%)", "毛利率(%)", "净利润率(%)", "股东人数"
        #               ],
        columns=["tsb.ts_code", "tsb.name", "tsb.industry", "tsb.area", "tdb.pe", "tdb.pb", "tdb.turnover_rate", "tdb.float_share", "tdb.total_share","tsb.list_date"],
        column_names=["代码", "名称", "所属行业", "地区", "市盈率","市净率","换手率", "流通股本(万股)", "总股本(万股)", "上市日期"],
        primary_key=[],
        order_by=" code asc "
    )
)
...
STOCK_WEB_DATA_LIST.append(
    StockWebData(
        ...
        columns=["index_code", "name", "weight"],
...
STOCK_WEB_DATA_LIST.append(
    StockWebData(
        ...
        columns=["date", "ts_code", "name", "pchange", "amount", "buy", "bratio", "sell", "sratio", "reason"],
...
STOCK_WEB_DATA_LIST.append(
    StockWebData(
        ...
        columns=["date", "ts_code", "name", "changepercent", "trade", "open", "high", "low", "settlement", "volume",
...
STOCK_WEB_DATA_LIST.append(
    StockWebData(
        ...
        columns=["date", "ts_code", "name", "change", "open", "preclose", "close", "high", "low", "volume", "amount"],
...
for tmp in STOCK_WEB_DATA_LIST:
    try:
        # 增加columns 字段中的【查看股票】
        if tmp.columns.__contains__('ts_code'):
            tmp_idx = tmp.columns.index("ts_code")
        elif tmp.columns.__contains__('index_code'):
            tmp_idx = tmp.columns.index("index_code")
        else:
            raise ('Invalid columns key')
